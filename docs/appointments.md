...
보안 활성화 시 기존 `OWNER_ADMIN`, `VET_ADMIN`, `ADMIN` 역할에 예약 API 접근을 허용합니다.
현재 DB에는 로그인 사용자와 보호자·수의사를 연결하는 FK가 없으므로 보호자 본인 예약만 허용하는 제한은 없습니다.
이는 기존 보호자·수의사 API의 역할 기반 접근 방식과 동일하며, 본인 예약 제한에는 사용자 연결 구조가 추가로 필요합니다.
